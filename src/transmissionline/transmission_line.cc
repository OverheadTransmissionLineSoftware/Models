...
void TransmissionLine::DeleteAlignmentPoint(const int& index) {
  is_updated_points_xyz_alignment_ = false;
  alignment_.DeletePoint(index);
}
...
Point3d TransmissionLine::PointXyzAlignment(const double& station) const {
  ...
  if (is_updated_points_xyz_alignment_ == false) {
    if (UpdatePointsXyzAlignment() == false) {
      return Point3d();
    }
  }
  ...
  // gets point
  return PointXyzAlignment(point_xyz_back, distance_station, distance_elevation,
...
Point3d TransmissionLine::PointXyzAlignment(
...
bool TransmissionLine::UpdatePointsXyzAlignment() const {
  ...
  for (auto iter = points->cbegin(); iter != points->cend(); iter++) {
    ...
    if (iter == points->cbegin()) {
      ...
      Point3d point = PointXyzAlignment(*point_xyz_prev,
                                        distance_station,
                                        distance_elevation,
